...
module.exports = async (req, res) => {
  try {
    if ( req.user && (req.user.level < 3)) {
      let {id_order_success_cancel,note} = req.body;
      ...
      await SuccessStatusToOrtherStatusOrderByOrderID(id_order_success_cancel,4,3,note);
      res.redirect('/admin/success_orders');
    } else {
      res.redirect('/admin/login');
    }
  } catch (e) {
    console.log("Loi ad_success_to_cancel_order: " + e);
  }
}
